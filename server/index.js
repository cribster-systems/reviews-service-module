...
if (cluster.isMaster) {
  ...
  app.get('/reviews/:locationId', function (req, res) {
      ...
      const redisKey = locationId + req.query.index + req.query.keyword;
        ...
        if (result) {
          ...
          db.find(locationId, function (err, results) {
              ...
              if (req.query.keyword) {     
                  ...
                  data = { getFive, totalReviews, searchResultsReviewsTotal };    
              } else {
                  searchResultsReviewsTotal = totalReviews;
                  data = { getFive, totalReviews, searchResultsReviewsTotal, averageRatings };
              }
              redisClient.setex(redisKey, 120, JSON.stringify(data));
              res.json(data);
          });
        }
      });
